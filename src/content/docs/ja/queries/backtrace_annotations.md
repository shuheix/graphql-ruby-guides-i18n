---
title: バックトレース注釈
description: デバッグに GraphQL のバックトレースを使用する
sidebar:
  order: 12
experimental: true
---
...
`backtrace` contains some execution data:
...
未処理のエラーを `GraphQL::Backtrace` を使って GraphQL エラーでラップできます。

この機能を `query` に対して有効にするには、`context` に `backtrace: true` を追加します。例えば:
...
あるいは、バックトレースを常にラップするには、`schema` 定義に `use` で追加します。例えば:
...
これで、未処理のエラーは `GraphQL::Backtrace::TracedError` によってラップされ、GraphQL のバックトレースも出力されます。例えば:
