---
title: インターフェース
description: Interfaces はオブジェクトが実装できる field の一覧です
sidebar:
  order: 4
redirect_from:
...
For example, let's say a `Customer` (interface) may be either an `Individual` (object) or a `Company` (object). Here's the structure in the [GraphQL スキーマ定義言語](https://graphql.org/learn/schema/#interface-types) (SDL):
