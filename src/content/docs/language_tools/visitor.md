---
title: AST Visitor
description: Analyze and modify parsed GraphQL code
sidebar:
  order: 0
---
...
[`GraphQL.parse`](https://graphql-ruby.org/api-doc/GraphQL.parse) turns a string into a GraphQL document:
...
Also, [`GraphQL.parse_file`](https://graphql-ruby.org/api-doc/GraphQL.parse_file) parses the contents of the named file and includes a `filename` in the parsed document.
...
To inspect or modify a parsed document, extend [`GraphQL::Language::Visitor`](https://graphql-ruby.org/api-doc/GraphQL::Language::Visitor) and implement its various hooks. It's an implementation of the [visitor pattern](https://en.wikipedia.org/wiki/Visitor_pattern). In short, each node of the tree will be "visited" by calling a method, and those methods can gather information and perform modifications.

In the visitor, each node class has a hook, for example:

- [`GraphQL::Language::Nodes::Field`](https://graphql-ruby.org/api-doc/GraphQL::Language::Nodes::Field)s are routed to `#on_field`
- [`GraphQL::Language::Nodes::Argument`](https://graphql-ruby.org/api-doc/GraphQL::Language::Nodes::Argument)s are routed to `#on_argument`

See the [`GraphQL::Language::Visitor`](https://graphql-ruby.org/api-doc/GraphQL::Language::Visitor) API docs for a full list of methods.
...
(These `.add_*` helpers are wrappers around [`GraphQL::Language::Nodes::AbstractNode#merge`](https://graphql-ruby.org/api-doc/GraphQL::Language::Nodes::AbstractNode#merge).)

## Print

The easiest way to turn an AST back into a string of GraphQL is [`GraphQL::Language::Nodes::AbstractNode#to_query_string`](https://graphql-ruby.org/api-doc/GraphQL::Language::Nodes::AbstractNode#to_query_string), for example:
...
You can also create a subclass of [`GraphQL::Language::Printer`](https://graphql-ruby.org/api-doc/GraphQL::Language::Printer) to customize how nodes are printed.
