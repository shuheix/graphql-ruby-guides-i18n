...
description: Checking permissions for mutations
sidebar:
  order: 3
---
...
- Add a prepare hook to fetch an object with the given `ID` (using [`Schema.object_from_id`](https://graphql-ruby.org/api-doc/Schema.object_from_id))
- Check that the fetched object's type matches the `loads:` type (using [`Schema.resolve_type`](https://graphql-ruby.org/api-doc/Schema.resolve_type))
