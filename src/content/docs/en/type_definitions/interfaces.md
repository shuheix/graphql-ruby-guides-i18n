---
title: Interfaces
description: Interfaces are lists of fields which objects may implement
sidebar:
  order: 4
redirect_from:
...
For example, let's say a `Customer` (interface) may be either an `Individual` (object) or a `Company` (object). Here's the structure in the [GraphQL Schema Definition Language](https://graphql.org/learn/schema/#interface-types) (SDL):
...
Interfaces are Ruby modules which include [`GraphQL::Schema::Interface`](https://graphql-ruby.org/api-doc/GraphQL::Schema::Interface). First, make a base module:
