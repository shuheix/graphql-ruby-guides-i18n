---
title: Scalars
description: Scalars are "simple" data types like integers and strings
sidebar:
  order: 1
---
...
- `ISO8601Duration`, an ISO 8601-encoded duration. ⚠ This requires `ActiveSupport::Duration` to be loaded and will raise [`GraphQL::Error`](https://graphql-ruby.org/api-doc/GraphQL::Error) if it's `.coerce_*` methods are called when it is not defined.
...
You can implement your own scalars by extending [`GraphQL::Schema::Scalar`](https://graphql-ruby.org/api-doc/GraphQL::Schema::Scalar). For example:
...
When incoming data is incorrect, the method may raise [`GraphQL::CoercionError`](https://graphql-ruby.org/api-doc/GraphQL::CoercionError), which will be returned to the client in the `"errors"` key.
