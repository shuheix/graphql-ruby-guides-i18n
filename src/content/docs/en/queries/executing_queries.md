...
description: Evaluate GraphQL queries with your schema
sidebar:
  order: 0
---


You can execute queries with your [`GraphQL::Schema`](https://graphql-ruby.org/api-doc/GraphQL::Schema) and get a Ruby Hash as a result. For example, to execute a query from a string:
...
- `document:` accepts an already-parsed query (instead of a string), see [`GraphQL.parse`](https://graphql-ruby.org/api-doc/GraphQL.parse)
...
Some of these options are described in more detail below, see [`GraphQL::Query#initialize`](https://graphql-ruby.org/api-doc/GraphQL::Query#initialize) for more information.
...
If the variable is a [`GraphQL::Schema::InputObject`](https://graphql-ruby.org/api-doc/GraphQL::Schema::InputObject), you can provide a nested hash, for example:
...
Note that `context` is _not_ the hash that you passed it. It's an instance of [`GraphQL::Query::Context`](https://graphql-ruby.org/api-doc/GraphQL::Query::Context), but it delegates `#[]`, `#[]=`, and a few other methods to the hash you provide.
...
[`GraphQL::Schema::Mutation`](https://graphql-ruby.org/api-doc/GraphQL::Schema::Mutation) fields will also receive `root_value:` as `obj` (assuming they're attached directly to your `MutationType`).
