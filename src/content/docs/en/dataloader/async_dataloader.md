...
description: Using AsyncDataloader to fetch external data in parallel
sidebar:
  order: 5
---

`AsyncDataloader` will run [`GraphQL::Dataloader::Source#fetch`](https://graphql-ruby.org/api-doc/GraphQL::Dataloader::Source#fetch) calls in parallel, so that external service calls (like database queries or network calls) don't have to wait in a queue.
...
Now, [`GraphQL::Dataloader::AsyncDataloader`](https://graphql-ruby.org/api-doc/GraphQL::Dataloader::AsyncDataloader) will create `Async::Task` instances instead of plain `Fiber`s and the `async` gem will manage parallelism.
