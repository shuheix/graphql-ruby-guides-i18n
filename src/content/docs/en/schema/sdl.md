...
description: Defining a schema from a string or .graphql file
sidebar:
  order: 7
---

GraphQL-Ruby includes a way to build a runable schema from the GraphQL Schema Definition Language (SDL). [`GraphQL::Schema.from_definition`](https://graphql-ruby.org/api-doc/GraphQL::Schema.from_definition) returns a schema class based on a filename or string containing GraphQL SDL. For example:
...
[`GraphQL::Schema.from_definition`](https://graphql-ruby.org/api-doc/GraphQL::Schema.from_definition) accepts a `using:` argument, which may be given as a map of `plugin => args` pairs. For example:
...
See [`GraphQL::Language::Nodes::Directive`](https://graphql-ruby.org/api-doc/GraphQL::Language::Nodes::Directive) for available methods.
