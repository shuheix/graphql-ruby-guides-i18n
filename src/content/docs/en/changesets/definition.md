...
description: Creating a set of modifications to release in an API version
sidebar:
  order: 2
enterprise: true
---
...
To add or redefine a field, use `field(..., added_in: ...)`, including all configuration values for the new implementation (see [`GraphQL::Schema::Field#initialize`](https://graphql-ruby.org/api-doc/GraphQL::Schema::Field#initialize)). The definition given here will override the previous definition (if there was one) whenever this Changeset applies.
