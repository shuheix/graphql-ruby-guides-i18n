...
Some clients may send _several_ queries to the server at once (for example, [Apollo Client's query batching](https://www.apollographql.com/docs/react/api/link/apollo-link-batch-http/)). You can execute them concurrently with [`Schema.multiplex`](https://graphql-ruby.org/api-doc/Schema.multiplex).
...
You can add values to [`Execution::Multiplex#context`](https://graphql-ruby.org/api-doc/Execution::Multiplex#context) by providing a `context:` hash:
...
Multiplex analyzers may return [`AnalysisError`](https://graphql-ruby.org/api-doc/AnalysisError) to halt execution of the whole multiplex.

## Multiplex Tracing

You can add hooks for each multiplex run with [trace modules](/queries/tracing).

The trace module may implement `def execute_multiplex(multiplex:)` which `yield`s to allow the multiplex to execute. See [`Execution::Multiplex`](https://graphql-ruby.org/api-doc/Execution::Multiplex) for available methods.
