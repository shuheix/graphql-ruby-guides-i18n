...
description: Using different schema members for each request
sidebar:
  order: 8
---
...
Also, you can implement `def arguments(context)` on your base field class to return a Hash of `{ String => GraphQL::Schema::Argument }` and `def get_argument(name, context)` to return a [`GraphQL::Schema::Argument`](https://graphql-ruby.org/api-doc/GraphQL::Schema::Argument) or `nil`. . If you take this approach, you might want some custom field classes for any types or resolvers that use these methods. That way, you don't have to reimplement the method for _all_ the fields in the schema.
...
Alternatively, you can implement `def self.enum_values(context)` in your enum types to return an Array of [`GraphQL::Schema::EnumValue`](https://graphql-ruby.org/api-doc/GraphQL::Schema::EnumValue)s. For example, to return a dynamic set of enum values:
...
To dump a certain _version_ of the schema, provide the applicable `context: ...` to [`Schema.to_definition`](https://graphql-ruby.org/api-doc/Schema.to_definition). For example:
