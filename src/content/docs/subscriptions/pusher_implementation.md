...
description: GraphQL subscriptions over Pusher
sidebar:
  order: 6
pro: true
---
...
By default, this is done with [`GraphQL::Subscriptions::Serialize`](https://graphql-ruby.org/api-doc/GraphQL::Subscriptions::Serialize)'s `dump` and `load` methods, but you can provide custom implementations as well. To customize the serialization logic, create a subclass of `GraphQL::Pro::PusherSubscriptions` and override `#dump_context(ctx)` and `#load_context(ctx_string)`:
