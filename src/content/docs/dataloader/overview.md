---
title: Overview
description: Getting started with the Fiber-based Dataloader
sidebar:
  order: 0
---

 [`GraphQL::Dataloader`](https://graphql-ruby.org/api-doc/GraphQL::Dataloader) provides efficient, batched access to external services, backed by Ruby's `Fiber` concurrency primitive. It has a per-query result cache and [AsyncDataloader](/dataloader/async_dataloader) supports truly parallel execution out-of-the-box.
...
To install [`GraphQL::Dataloader`](https://graphql-ruby.org/api-doc/GraphQL::Dataloader), add it to your schema with `use ...`, for example:
